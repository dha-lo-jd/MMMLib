...
public class MMM_GuiTextureSlot extends GuiSlot {
	...
	private ItemStack armors[] = new ItemStack[] {
			new ItemStack(Item.bootsLeather), new ItemStack(Item.legsLeather), new ItemStack(Item.plateLeather), new ItemStack(Item.helmetLeather)
	};
	...
	public MMM_GuiTextureSlot(MMM_GuiTextureSelect pOwner) {
		...
		blankBox.models = new MMM_ModelMultiBase[] {
				null, null, null
		};
	...
	public void drawSlot(int var1, int var2, int var3, int var4, Tessellator var5) {
		...
		if (mode) {
			...
			if (lmap != null) {
				...
				// lbox.getArmorTextureName(MMM_TextureManager.tx_armor1,
				// "default", 0);
				// ltxname = entity.getTextures(2);
				// ltxname[0] = ltxname[1] = ltxname[2] = ltxname[3] =
				// lbox.getArmorTextureName(MMM_TextureManager.tx_armor2,
				// "default", 0);
			...
			for (int li = 0; li < 16; li++) {
				...
				if (lbox.hasColor(li, isContract)) {
					...
					// entity.getTextures(0)[0] = lbox.getTextureName(li +
					// (isContract ? 0 : MMM_TextureManager.tx_wild));
					RenderManager.instance.renderEntityWithPosYaw(entity, 0.0D, 0.0D, 0.0D, 0.0F, 1.0F);
					MMM_Client.setLightmapTextureCoords(0x00f0);// 61680
				}
			}

...
public class MMM_Helper {
	...
	/**
	 * 変数「avatar」から値を取り出し戻り値として返す。 avatarが存在しない場合は元の値を返す。 avatarはEntityLiving互換。
	 */
	...
	/**
	 * 変数「maidAvatar」から値を取り出し戻り値として返す。 maidAvatarが存在しない場合は元の値を返す。
	...
	/**
	 * 16進数の文字列をIntへ変換する。 0xffffffff対策。
	 * 
	 * @param pValue
	 * @return
	 */
	...
	/**
	 * Blockのインスタンスを置き換える。 static finalの変数に対して行うのでForgeでは無効。
	 * 
	 * @param pOriginal
	 * @param pReplace
	 * @return
	...
	/**
	 * マルチ対応用。 ItemStackに情報更新を行うと、サーバー側との差異からSlotのアップデートが行われる。
	...
	/**
	 * バイオームの設定Entityを置き換えられたEntityへ置き換える。 基本的にMMMLib以外からは呼ばれない。
	 */
	...
	/**
	 * Modloader環境下で空いているEntityIDを返す。 有効な値を獲得できなければ-1を返す。
	 */

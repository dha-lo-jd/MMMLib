...
public class MMM_ItemRenderManager {
	...
	public static MMM_ItemRendererForge forgeRender;
	...
	public static void registerForge(Item pItem, MMM_IItemRenderManager pEXRender) {
		...
		try {
			...
			// MinecraftForgeClient.registerItemRenderer(pItem.itemID,
			// forgeRender);
			e.printStackTrace();
		}
	...
	public Method frenderItem;

	public Method frenderItemInFirstPerson;

	public Method fgetRenderTexture;

	public Method frenderItemWorld;

	public MMM_IItemRenderManager exRender;
	...
	private MMM_ItemRenderManager(Object pObject, MMM_IItemRenderManager pEXRender) {
		fobject = pObject;
		exRender = pEXRender;
		// frenderItem = pEXRender.getClass().getDeclaredMethod("renderItem",
		// EntityLivingBase.class, ItemStack.class, int.class);
		// frenderItemInFirstPerson = prenderItemInFirstPerson;
		// fgetRenderTexture = pgetRenderTexture;
		// frenderItemWorld = prenderItemWorld;
		random = new Random();
	...
	public boolean renderItemInFirstPerson(Entity pEntity, ItemStack pItemStack, float pDeltaTimepRenderPhatialTick) {
		// public boolean renderItemInFirstPerson(float pDelta, MMM_ItemRenderer
		// pItemRenderer) {
		if (exRender != null) {
	...
	public void renderItemLocal(EntityLivingBase entityliving, ItemStack itemstack, int i) {
		...
		if (lflag) {
			if (itemstack != null && itemstack.hasEffect() && i == 0) {
				...
				// renderItemIn2D(var6, 0.0F, 0.0F, 1.0F, 1.0F, 256, 256,
				// 0.0625F);
				GL11.glPopMatrix();
	...
	public boolean renderItemWorld(EntityItem entityitem, double d, double d1, double d2, float f, float f1) {
		...
		random.setSeed(187L);
		GL11.glPushMatrix();
		float f2 = MathHelper.sin((entityitem.age + f1) / 10F + entityitem.hoverStart) * 0.1F + 0.1F;
		float f3 = ((entityitem.age + f1) / 20F + entityitem.hoverStart) * 57.29578F;
		byte byte0 = 1;
		if (lis.stackSize > 1) {
			byte0 = 2;
		}
